...
const PendingChats = () => {
  ...
  const {
    ...
    queryKey: ['all-default-chats-with-customer'],
  ...
  useEffect(() => {
    if (defaultChatsData?.data) {
      console.log('Default Chats Data:', defaultChatsData.data);
      chatIdsRef.current = defaultChatsData.data.map((chat) => chat.id);
      const filteredChats = applyFilters(defaultChatsData.data);
      setDisplayChats(filteredChats);
    }
  }, [defaultChatsData, searchTerm, selectedCategory]);
